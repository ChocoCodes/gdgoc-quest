...
const useGSAPAnimations = () => {
  ...
  const animateQuestBoardEntrance = () => {
  ...
  const animateQuestHover = (element: HTMLElement) => {
    gsap.to(element, {
      scale: 1.02,
      rotationX: 1,
      y: -2,
  ...
  const animateQuestHoverOut = (element: HTMLElement) => {
    gsap.to(element, {
      scale: 1,
      rotationX: 0,
      y: 0,
  ...
  const animateModalExit = (onComplete?: () => void) => {
    if (!modalRef.current || !modalBackdropRef.current) return
  ...
  const animateQuestCompletion = (element: HTMLElement) => {
    const tl = gsap.timeline()
  ...
  const animateInkWriting = (textElement: HTMLElement, text: string) => {
  ...
  const addQuestItemRef = (el: HTMLDivElement | null, index: number) => {
    if (el) {
