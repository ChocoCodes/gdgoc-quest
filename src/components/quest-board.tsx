...
const QuestBoard = ({ quest, onQuestClick }: QuestBoardProps) => {
  ...
  return (
    <div className="relative w-full max-w-full sm:max-w-4xl md:max-w-5xl mx-auto px-4">
      <ParchmentBackground ref={questBoardRef} className="py-16 px-4 sm:px-8 md:px-12 lg:px-16">
        ...
        <div className="w-full max-w-xl px-4 sm:px-8 mb-12">
          ...
          >
            ...
            >
              ...
              {quest.isGrandPrize ? (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: "inset 0 2px 4px rgba(255, 255, 255, 0.3), 0 6px 12px rgba(255, 193, 7, 0.6)",
                  }}
                >
                  <span className="text-yellow-900 font-bold text-md sm:text-lg font-jacquard-display">👑</span>
                </div>
              ) : (
                <div
                ...
                >
                  <span className="text-amber-100 font-bold text-md sm:text-lg font-jacquard-display">I</span>
                </div>
              )}
              ...
              <div className="relative p-6 sm:p-8 md:p-10">
                ...
                <div className="flex items-center justify-center">
                  {quest.completed ? (
                    ...
                    <div
                      className={`flex items-center gap-3 ${quest.isGrandPrize ? "text-yellow-800" : "text-amber-800"}`}
                    >
                      <div
                        className={`w-8 h-8 border-3 ${quest.isGrandPrize ? "border-yellow-800 bg-yellow-50" : "border-amber-800 bg-amber-50"} rounded-full flex items-center justify-center shadow-inner`}
                      >
                        <div
                          className={`w-3 h-3 ${quest.isGrandPrize ? "bg-yellow-800" : "bg-amber-800"} rounded-full`}
                        ></div>
                      </div>
              ...
              <div
                className={`absolute top-4 left-4 w-8 h-8 border-l-3 border-t-3 ${quest.isGrandPrize ? "border-yellow-800" : "border-amber-800"} opacity-40`}
              ></div>
              <div
                className={`absolute top-4 right-4 w-8 h-8 border-r-3 border-t-3 ${quest.isGrandPrize ? "border-yellow-800" : "border-amber-800"} opacity-40`}
              ></div>
              <div
                className={`absolute bottom-4 left-4 w-8 h-8 border-l-3 border-b-3 ${quest.isGrandPrize ? "border-yellow-800" : "border-amber-800"} opacity-40`}
              ></div>
              <div
                className={`absolute bottom-4 right-4 w-8 h-8 border-r-3 border-b-3 ${quest.isGrandPrize ? "border-yellow-800" : "border-amber-800"} opacity-40`}
              ></div>

              <div
                className={`absolute top-8 right-12 w-2 h-2 ${quest.isGrandPrize ? "bg-yellow-700" : "bg-amber-700"} rounded-full opacity-10`}
              ></div>
              <div
                className={`absolute bottom-12 left-8 w-1 h-1 ${quest.isGrandPrize ? "bg-yellow-800" : "bg-amber-800"} rounded-full opacity-15`}
              ></div>
            </div>
